    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if event.key == pygame.K_d and direction != 3:
                direction = 4
        if game_over:
            img = pygame.font.SysFont(None, 40).render('Game Over!', True, RED)
            screen.blit(img, ((WINDOW_WIDTH//2)-(img.get_width()/2), (WINDOW_HEIGHT//2)-(img.get_height()/2)))

            
            restart_img = pygame.font.SysFont(None, 30).render('Play again?', True, CYAN)
            screen.blit(restart_img, ((WINDOW_WIDTH//2)-(restart_img.get_width()/2), (WINDOW_HEIGHT//2)-(restart_img.get_height()/2)+(img.get_height()*3)))
            
            pygame.draw.rect(screen, RED, ((WINDOW_WIDTH//2)-(restart_img.get_width()/2), (WINDOW_HEIGHT//2)-(restart_img.get_height()/2)+(img.get_height()*3)))
            # Click play again?
